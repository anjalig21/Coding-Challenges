...
(define (build-dt examples label)
  (local [(define attributes (collect-attributes examples))
          ...
          (define (same-subtree? sub-1 sub-2 label)
            (cond [(equal? sub-1 sub-2) sub-1]
                  [else (list label sub-1 sub-2)]))]
    ...
    (cond [(empty? examples) empty]
          ...
          [(empty? (collect-attributes examples)) (cond [(> (length (first (split-examples examples label)))
                                                                ...
                                                                [else false])]
          [else (cons (same-subtree? (build-dt (remove-symbol (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split))))) (first (split-examples examples (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split)))))))) label)
                                     (build-dt (second (split-examples examples (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split))))))) label)
                                     (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split))))))
                      (cons (build-dt (remove-symbol (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split))))) (first (split-examples examples (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split)))))))) label)
                      (list (build-dt (second (split-examples examples (best-attribute (entropy-attributes (augment-histogram label-histogram attributes (length (first split)))
                                                           (augment-histogram not-label-histogram attributes (length (second split))))))) label))))])))